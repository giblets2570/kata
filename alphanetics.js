...
let answer = { Y: 4, S: 2, R: 8, E: 9, A: 5, O: 3, N: 1, I: 0, B: 7, Z: 6 };
...
function alphametics(s) {
  ...
  function fn (letterToNumber, depth) {
    ...
    if (!defined(letterToNumber[rightWord[index]])) {
      ...
      let carry = parseInt(leftWordsPrevSlice
          .map((w) => wordToNumber(w, letterToNumber))
          .reduce((c, w) => c + w, 0)/Math.pow(10, depth-1));
      ...
      // console.log(permutations.length, depth)
      permutations = permutations
        ...
        .filter(p => {
          ...
          sum = sum.map((l) => nextLetterToNumber[l]).filter(l => l);
