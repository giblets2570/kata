function SQLEngine(database){
  ...
  this.execute = function(query){
    ...
    console.log(query);
    // From
    let match = query.match(/from\s+[a-z0-9_]+(\s+|$)/i);
    if (!match) return false;
    let table = match[0].split(' ')[1];

    // Select
    match = query.match(/select\s+([a-z.0-9_,])*[a-z.0-9_]+\s+/i);
    if (!match) return false;
    let columns = match[0].split(' ')[1].split(',');

    // Wheres
    let filters = [];
    match = query.match(/where\s+[a-z.0-9\u00E0-\u00FC_,=<>:'\- ]+/i);
    if (match) {
      match = match[0]
      let filter = match.replace(/where\s+/i, '');
      if (filter.includes('<>')) {
        let [col, val] = filter.split(/<>/i);
    ...
    let data = database[table].map((row) => {
      return Object.keys(row).reduce((c, col) => {
        c[table+'.'+col] = row[col];
        return c;
      }, {});
    });
    ...
    // Join
    let usedTables = new Set([table]);
    // console.log(Object.keys(data[0]))
    let joinMatches = query.match(/join\s+[a-z0-9_]+\s+on\s+[a-z.0-9_,=]+/gi);
    if (joinMatches) {
      for(let joinMatch of joinMatches) {
        // console.log('joinMatch:', joinMatch)
        let joinTable = joinMatch.match(/join\s+[a-z0-9_]+ /i)[0].split(' ')[1];
        let onValues = joinMatch.match(/on\s+[a-z.0-9_,=]+$/i)[0].replace(/on\s+/i, '');
        let [source, dest] = onValues.split(/=/i);
        if (usedTables.has(dest.split('.')[0])) {
          let temp = dest;
          dest = source;
          source = temp;
        }
        let [destTable, destCol] = dest.split('.');
        usedTables.add(destTable);
        let destData = database[destTable].map((row) => {
          return Object.keys(row).reduce((c, col) => {
            c[destTable+'.'+col] = row[col];
            return c;
          }, {});
        });
        if (destData.length > data.length) {
          let hashMap = data.reduce((c, row) => {
            if (c[row[source]]) return c;
            c[row[source]] = Object.assign({}, row);
            return c;
          }, {});
          data = destData.map((row) => {
            row = Object.assign({}, row, hashMap[String(row[dest])])
            return row;
          });
        } else {
          let hashMap = destData.reduce((c, row) => {
            if (c[row[dest]]) return c;
            c[row[dest]] = Object.assign({}, row);
            return c;
          }, {});
          data = data.map((row) => {
            row = Object.assign({}, row, hashMap[String(row[source])])
            return row;
          });
        }
      }
    ...
    return data
      .filter((row) => {
        for(let [col,val,op] of filters) {
          if (op === '=') {
            if (String(row[col]) !== this.parseString(val)) return false;
          } else if (op === '<>') {
            if (String(row[col]) === this.parseString(val)) return false;
          } else if (op === '>=') {
            if (row[col] < parseFloat(val)) return false;
          } else if (op === '<=') {
            if (row[col] > parseFloat(val)) return false;
          } else if (op === '>') {
            if (row[col] <= parseFloat(val)) return false;
      ...
      .map((row) => {
        ...
        for (let i in columns) {
          result[columns[i]] = row[columns[i]];
        }
        return result;
      });
  }
...
// var actual = engine.execute('SELECT movie.name,movie.directorID FROM movie');
// console.log(actual);
//
...
// var actual = engine.execute('SELECT movie.name,director.name FROM movie JOIN director ON director.id=movie.directorID');
// console.log(actual);
// var actual = engine.execute('SELECT movie.name,director.name FROM movie JOIN director ON movie.directorID=director.id');
// var actual = engine.execute('SELECT movie.name,director.name FROM director JOIN movie ON director.id=movie.directorID');
// console.log(actual);
// var actual = engine.execute(`SELECT movie.name FROM movie WHERE movie.name='Pirates of the Caribbean: Dead Man''s Chest'`);
// console.log(actual);
// var actual = engine.execute("select movie.directorID,movie.id,actor.id from actor join actor_to_movie on actor_to_movie.actorID=actor.id JOIN movie ON movie.id=actor_to_movie.movieID WHERE actor.name='Gary Oldman'");
// console.log(actual);
var actual = engine.execute("select director.name from actor join actor_to_movie ON actor_to_movie.actorID=actor.id JOIN movie on movie.id=actor_to_movie.movieID JOIN director on director.id=movie.directorID where actor.name='Penélope Cruz'");
console.log(actual);
